...
RSpec.describe Jet::Client, '#get_token' do
  context "asks for token" do
    it "returns token header" do
      ...
      submit_body = {user: 'foo', pass: 'bar'}.to_json
  ...
  context "asks for token with bad creds" do
    it "raises 400 error" do
      submit_body = {user: 'notreal', pass: 'badcreds'}.to_json
      ...
      expect { client.get_token }.to raise_error(RestClient::BadRequest)
    end
  end
end
